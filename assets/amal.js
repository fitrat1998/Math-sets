function rnd(n1, n2, kmax, stxt)	//Генератор	последовательности случайных
{
    ...
    } while (knum < kmax) eval("document.all.txt" + stxt + ".value=s")
    f2(); 
    fsort(stxt);
}
...
function g(k) //Сведение общей задачи к более элементарным
{
    var ind = new Array();
    ind[0] = 'Объединить';
    ind[1] = 'Разность А и В';
    ind[2] = 'Разность В и А';
    ind[3] = 'Пересечение';
    ind[4] = 'Симметричная разность';
...
{
    ma = document.all.txt1.value; mb = document.all.txt2.value; if (n == 2) { temp = ma; ma = mb; mb = temp; } ma = ma.split(' ');
    mb = mb.split(" ");
